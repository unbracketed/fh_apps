...
def __ft__(self: Event):
        return Card(
            ...
            Div(cls="mt-2")(
                A(href=f"/event/{self.id}", cls="btn btn-primary text-blue-500 hover:underline mr-4")(
                    "View Details"
                ),
                A(href=f"/edit_event/{self.id}", cls="btn btn-secondary text-green-500 hover:underline")(
                    "Edit",
                ),
            ),
...
def EventDetails(event: Event):
    return Div(
        ...
        P(f"Start Time: {event.start_time}", cls="text-xl font-semibold") if event.start_time else "",
        P(f"Venue: {event.venue}", cls="mb-2") if event.venue else "",
        P(event.description, cls="mt-4") if event.description else "",
        A("Event URL", href=event.url, cls="text-blue-500 hover:underline") if event.url else "",
        Div(
            A("Back to Events", href="/",
              cls="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600"),
            A("Edit Event", href=f"/edit_event/{event.id}",
